 ...
 *   2. Distance/volume pic + progression
 *   3. SL S1
 *   4. Faisabilité (R2 gates)
...
// Recherche email avec variantes TLD (utilise findUserByEmail, séquentiel)
// baseEmail = "arenaarmando@hotmail" ; domains = ["com","fr",...]
...
log('═'.repeat(125));
log(`  AUDIT 3 NOUVEAUX CLIENTS S1 (18/05/2026)  •  ${new Date().toLocaleString('fr-FR')}`);
log('═'.repeat(125));
...
const armandoEmail = armandoVariants[0]?.email;
if(!armandoEmail){
  log(`  ❌ Aucune variante trouvée pour arenaarmando — abandon`);
}

const CLIENTS = [
  { tag: 'Client 1', email: 'antoineg.gde@outlook.fr', prenomAttendu: '(antoineg)' },
  { tag: 'Client 2', email: 'nabou57@hotmail.fr', prenomAttendu: 'Annabelle' },
];
if(armandoEmail){
  CLIENTS.push({ tag: 'Client 3', email: armandoEmail, prenomAttendu: 'Armando' });
}
...
for(const c of CLIENTS){
  ...
  const uid = u.localId;
  log(`  UID: ${uid}  •  Créé: ${u.createdAt?new Date(parseInt(u.createdAt)).toISOString():'?'}  •  Email vérifié: ${u.emailVerified?'oui':'non'}`);
  ...
  // On prend de préférence le plan complet (fullPlanGenerated=true) le plus récent
  ...
  log(`  Profil: ${age?age+' ans':'?'} • IMC ${bmi?.toFixed?.(1)||'?'} • Niveau: ${level||'?'} • VMA: ${vma?.toFixed?.(2)||'?'} km/h (${p.vmaSource||'?'})`);
  log(`  Vol actuel: ${curVol||'?'} km/sem${curDplus?` / ${curDplus}m D+`:''}  •  Blessures: ${fmtInj(inj)}`);
  if(Object.keys(recent).length) log(`  Chronos récents: ${Object.entries(recent).map(([k,v])=>`${k}=${v}`).join(' • ')}`);

  // ─── 1. ALLURES ───
  log(`\n  ━━━ 1. ALLURES COHÉRENCE vs OBJECTIF ━━━`);
  log(`    Allures théoriques (paces[]) :`);
  for(const [k,v] of Object.entries(paces)) log(`      ${k.padEnd(30)} ${v}/km`);
  ...
  if(targetTime && vma){
    ...
    if(raceKm && chronoSec){
      ...
      log(`    Cohérence chrono cible :`);
      log(`      ${lbl} ${raceKm}km en ${targetTime} → ${chronoCheck.paceStr} = ${(pctVMA*100).toFixed(0)}%VMA  →  ${verdict}  (att. ${(expected[0]*100).toFixed(0)}-${(expected[1]*100).toFixed(0)}%VMA)`);
    }
  } else {
    log(`    Cohérence chrono cible : Finisher / pas de cible chrono`);
  }
  ...
  for(const s of sess){
    if(!s.targetPace) {
      log(`      ⚪ ${(s.day||'?').padEnd(10)} ${(s.type||'?').padEnd(18)} ${(s.intensity||'').padEnd(15)} (pas de targetPace) — "${s.title||''}"`);
      continue;
    }
    ...
    log(`      ${status} ${(s.day||'?').padEnd(10)} ${(s.type||'?').padEnd(18)} pace=${pace}/km ${expectedKey?`vs ${expectedKey}=${expected}`:''}${diffStr} — "${s.title}"`);
  }

  // ─── 2. DISTANCES / VOLUME PIC ───
  log(`\n  ━━━ 2. DISTANCE / VOLUME PIC (progression projetée) ━━━`);
  ...
  log(`    Total km course S1 : ${distKmS1.toFixed(1)} km`);
  log(`    SL S1 max : ${slDistMax} km${slElevMax?` / ${slElevMax}m D+`:''}`);
  if(slDistMax && wv[0]) {
    const ratio = slDistMax/wv[0]*100;
    const flag = ratio>40?'🔴':ratio>30?'🟡':'✅';
    log(`    Ratio SL/Volume hebdo S1 : ${ratio.toFixed(0)}% ${flag} (cible 30-40% max)`);
  }
  if(slDistMax && curVol){
  ...
  const forbiddenWords = ['poids', 'imc', 'minceur', 'silhouette', 'kg perdu', 'kilos', 'maigrir', 'amincir'];
  const found = forbiddenWords.filter(w => lowerWm.includes(w));
  log(`      Mots interdits poids/IMC/minceur dans corps : ${found.length?`🔴 TROUVÉ : ${found.join(', ')}`:'✅ aucun'}`);
  ...
  dumps[c.tag] = {
    ...
    plan: { id: p.id, name: p.name, createdAt: p.createdAt, durationWeeks: p.durationWeeks, sessionsPerWeek: p.sessionsPerWeek },
    inputs: { goal, subGoal, targetTime, raceDate, freq, vma, level, curVol, curDplus, inj, age, bmi, trailDist, trailElev, recent },
    paces,
...
// ─── TABLEAU SYNTHÈSE ───
log('\n\n' + '═'.repeat(125));
log(`  📋 TABLEAU SYNTHÈSE — 3 NOUVEAUX CLIENTS`);
log('═'.repeat(125));
log(`  ${'Prénom'.padEnd(12)} ${'Goal'.padEnd(22)} ${'Cible'.padEnd(10)} ${'Niv.'.padEnd(8)} ${'VMA'.padEnd(5)} ${'Vol'.padEnd(5)} ${'Jump'.padEnd(7)} ${'Pic'.padEnd(10)} ${'MaxΔ'.padEnd(15)} ${'Affût'.padEnd(7)} ${'SL'.padEnd(6)} ${'All'.padEnd(5)} ${'Chrono'.padEnd(18)} Fiab/score`);
log(`  ${'─'.repeat(123)}`);
for(const r of synthClients){
  log(`  ${(r.firstName||'?').substring(0,12).padEnd(12)} ${(r.goal).substring(0,22).padEnd(22)} ${String(r.target).substring(0,10).padEnd(10)} ${String(r.level||'?').substring(0,8).padEnd(8)} ${String(r.vma).padEnd(5)} ${String(r.curVol).padEnd(5)} ${String(r.startJump).padEnd(7)} ${(`${r.peakKm}/S${r.peakWeek}`).padEnd(10)} ${String(r.maxJump).padEnd(15)} ${String(r.taperPct).padEnd(7)} ${(r.slS1Max+'km').padEnd(6)} ${r.allureOK.padEnd(5)} ${r.chronoVerdict.substring(0,18).padEnd(18)} ${r.fStatus}/${r.fScore}`);
}

writeFileSync('audit-3-nouveaux-s1.txt', out.join('\n'));
writeFileSync('audit-3-nouveaux-s1.json', JSON.stringify(dumps, null, 2));
console.log('\n📝 audit-3-nouveaux-s1.txt');
console.log('📝 audit-3-nouveaux-s1.json');
