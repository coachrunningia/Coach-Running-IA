...
const AuthModal: React.FC<AuthModalProps> = ({ onAuthSuccess, onClose, isModal = false, initialEmail = '' }) => {
  ...
  const content = (
    <div className={`bg-white rounded-2xl p-8 w-full max-w-md ${isModal ? 'shadow-2xl relative' : 'shadow-none'}`}>
      ...
      {/* Mobile iOS J2 (02/06/2026) — Google Sign-In masqué en iOS natif.
          Stratégie V1 : pas de Sign-In tiers en iOS = pas d'obligation Apple
          Sign-In (guideline 5.1.1). Apple Sign-In natif sera V1.1 (plugin
          @capacitor-community/apple-sign-in requis pour ASAuthorizationController).
          NB : signInWithPopup Firebase ne marche pas en WebView Capacitor iOS,
          donc le bouton Google ne fonctionnait probablement déjà pas en natif. */}
      {!isIOSNative && (
        <>
          <button
            type="button"
