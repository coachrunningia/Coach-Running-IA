...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, setUser }) => {
  ...
  const handlePortal = async () => {
    ...
    try {
      ...
      // If createPortalSession succeeds, the page navigates away (window.location.assign)
      // so we don't need to reset loading. But add a safety timeout just in case:
      setTimeout(() => setLoading(false), 5000);
    } catch (e: any) {
      console.error(e);
      setError(e?.message || "Impossible d'accéder au portail de facturation.");
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        ...
        <div className="mb-8 p-6 rounded-xl border border-slate-200 bg-slate-50/50">
            ...
            {user.isPremium ? (
              // CAS: PREMIUM
              <div>
                 {/* Statut premium avec info résiliation si applicable */}
                 {user.premiumCancelAt ? (
                   <div className="flex items-center gap-3 mb-4 bg-amber-50 p-4 rounded-lg border border-amber-200">
                      <div className="bg-amber-100 p-2 rounded-full text-amber-700">
                         <Clock size={24} />
                      </div>
                      <div>
                         <p className="font-bold text-amber-800">PREMIUM — Résiliation programmée</p>
                         <p className="text-xs text-amber-700">
                            Votre accès Premium reste actif jusqu'au{' '}
                            <span className="font-bold">
                              {new Date(user.premiumCancelAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
                            </span>.
                         </p>
                 ...
                 {user.stripeCustomerId ? (
                     ...
                     >
                        {loading ? 'Chargement...' : (
                          <>
                            ...
                            {user.premiumCancelAt ? 'Gérer mon abonnement (Réactiver / Modifier)' : 'Gérer mon abonnement (Résilier / Changer)'}
                          </>
                        )}
                     </button>
                   ...
                   <div className="bg-amber-50 p-3 rounded-lg border border-amber-200 text-center">
                     <p className="text-xs text-amber-700">
                       Votre paiement est en cours de synchronisation. La gestion de l'abonnement sera disponible dans quelques instants.
                     </p>
                     <button
                       onClick={() => window.location.reload()}
                       className="mt-2 text-xs text-amber-800 underline hover:text-amber-900 font-medium"
                     >
                       Rafraîchir la page
                     </button>
                   </div>
            ...
            ) : user.premiumCancelledAt ? (
              // CAS: ANCIEN PREMIUM (résilié)
              <div>
                 <div className="flex items-center gap-3 mb-4 bg-slate-100 p-4 rounded-lg border border-slate-200">
                    ...
                    <div>
                       <p className="font-bold text-slate-700">Abonnement résilié</p>
                       <p className="text-xs text-slate-500">
                          Résilié le {new Date(user.premiumCancelledAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}.
                          Accès limité (1ère semaine des plans uniquement).
                       </p>
                    </div>
                 </div>
                 <button
